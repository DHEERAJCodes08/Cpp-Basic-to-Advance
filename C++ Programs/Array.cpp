...
//Create an array taking inputs from the User and Providing the Max and Min values

#include<iostream>
#include<climits>             //this is Used because we use INT_MAX , INT_MIN   
using namespace std;

int main(){
    int n;
    int A[n];
    cout<<"Enter the Size of the A ::";
    cin>>n;
    cout<<"\nEnter the ellements in the Array ::";
    for( int i=0;i<=n;i++){
        cin>>A[i];
    } 
    ...
    for(int i=0;i<=n;i++){
       /*  if (A[i]>maxNo)                  //Insted of writing this 
                                              
        {
            maxNo=A[i];                 //We can use an built in Function maxNo=max(maxNO , A[i]);
        ...
        /* if (A[i]<minNo)                 //Insted of writing this
        {
            minNo=A[i];                //We can use an built in Function minNo=min(minNO , A[i]);
        }    */
